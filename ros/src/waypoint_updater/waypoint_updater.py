...
class WaypointUpdater(object):
    def __init__(self):
        rospy.loginfo('Initializing waypoint updaater')
...
#         rospy.Subscriber('/obstacle_waypoint', Int32, self.obstacle_cb)
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            rospy.loginfo('Before publishing waypoints')
            if self.pose and self.waypoint_tree:
                #closest_waypoint_idx = self.get_closest_waypoint_idx()
                #self.publish_waypoints(closest_waypoint_idx)
                self.publish_waypoints()
                rospy.loginfo('After publishing waypoints')
            rate.sleep()
...
#         self.final_waypoints_pub.publish(lane)
    def publish_waypoints(self):
        rospy.loginfo('publishing waypoints right now')
        final_lane = self.generate_lane()
        self.final_waypoints_pub.publish(final_lane)

    def generate_lane(self):
        rospy.loginfo('Generating lane')
        lane = Lane()
        ...
        if self.stopline_wp_idx == -1 or (self.stopline_wp_idx >= farthest_idx):
            ...
            rospy.loginfo('closer to waypoints may be decelerating. closest_idx = ' + str(closest_idx))
            lane.waypoints = self.decelerate_waypoints(base_waypoints, closest_idx)

        rospy.loginfo('Returning the lane')
    ...
    def decelerate_waypoints(self, waypoints, closest_idx):
        ...
        for i, wp in enumerate(waypoints):
            ...
            rospy.loginfo('distance to nearest waypoint closer to stop idx = ' + str(dist))
            rospy.loginfo('stop idx = ' + str(stop_idx))
    ...
    def traffic_cb(self, msg):
        ...
        rospy.loginfo("stopline_wp_idx = " + str(self.stopline_wp_idx))
        pass
